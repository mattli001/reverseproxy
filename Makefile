...
PKG_SRC := github.com/mattli001/reverseproxy
...
build:
	...
	GOARCH=amd64 GOOS=linux CGO_ENABLED=0 go build -ldflags "-s -w" -ldflags "-X $(PKG_SRC)/cmd.version=${VERSION}" -o "dist/apiproxy_linux-amd64" $(PKG_SRC)
